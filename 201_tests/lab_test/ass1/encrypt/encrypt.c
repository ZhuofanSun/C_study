...
void check_mapf_format(char * mapping_file, char pt_text[], char ct_text[]){
    ...
    while(fscanf(map_f, "%c,%c\n", &curr_pt, &curr_ct) == 2 && curr_lines <= 26){
...
int main(int argc, char * argv[]) {
    // this will store mapping_file name, encryption_mode, input_file name in that order, read during check usage
    char * mf_em_if[3];
    char pt_text[26], ct_text[26];  // store the plaintext alphabet and corresponding ciphertext alphabetfead
